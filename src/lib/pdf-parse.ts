...
export async function _parsePdf(selectedFile: string) {
    try {
        ...
        //throw error(500, "Failed to parse PDF" + err);
        console.error(err);
    }
}

/**
 * Count the words in a Document and return the top 20 most frequent words
 * @param inputString
 */
function countWordsAndSort(inputString: string) {
    ...
    wordCountPairs.slice(0, 20).forEach(pair => {
...
function getKeyWordsfromPDF(DocumentRecord :any ) {
    ...
    DocumentRecord.map(function (value :string) {
        ...
        words.push(keyword_extractor.extract(value.pageContent, {
            language: "english",
            remove_digits: true,
